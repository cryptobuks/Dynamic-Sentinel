...
class DynamicConfig():
    ...
    def slurp_config_file(self, filename):
        # read dynamic.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is dynamic.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from dynamic.conf
        ...
        # convert to an int if taken from dynamic.conf
